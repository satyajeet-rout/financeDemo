...
const OutputStep = ({ onBack }) => {
  // Array of output items - using your actual files from assets folder
  const outputs = [
    { 
      id: 1, 
      ...
      filePath: '/assets/csv/broker_performance.csv'
    },
    { 
      id: 2, 
      name: 'Sales Summary Comprehensive', 
      type: 'csv',
      fileName: 'sales_summary_comprehensive.csv',
      filePath: '/assets/csv/sales_summary_comprehensive.csv'
    },
    { 
      id: 3, 
      name: 'Region Property Matrix', 
      type: 'excel',
      fileName: 'region_property_matrix.xlsx',
      filePath: '/assets/excel/region_property_matrix.xlsx'
    },
    { 
      id: 4, 
      name: 'Sales Performance Management', 
      type: 'ppt',
      fileName: 'Sales_Performance_Management_Presentation.pptx',
      filePath: '/assets/presentations/Sales_Performance_Management_Presentation.pptx'
    }
  ];
  ...
  // Handle download function for actual files
  const handleDownload = (output) => {
    try {
      console.log(`Attempting to download: ${output.filePath}`);
      
      // Create download link for the actual file
      const a = document.createElement('a');
      a.href = output.filePath;
      ...
      alert(`Error downloading ${output.fileName}. Please check if the file exists in the assets folder.`);
    }
  };
    ...
    <div className="px-6 py-6">
      ...
      <div className="mb-6 bg-blue-50 p-4 rounded-lg border border-blue-100">
        <p className="text-sm text-gray-700 leading-relaxed">
          Preview and download your outputs here. When you're ready, tag each output as 'final' to lock it for editing and publish it to your report space.
        </p>
      </div>

      <div className="flex justify-between items-center mb-4">
        <div className="text-gray-700">
          <span className="font-medium">Outputs 4/4</span>
        </div>
      </div>
      ...
      {/* Output Grid - Using 4 columns for the 4 files */}
      <div className="grid grid-cols-4 gap-6">
        {outputs.map((output) => (
          <div 
            key={output.id} 
            className="bg-white border border-gray-200 rounded-xl py-4 px-3 flex flex-col relative shadow-sm w-full"
            onMouseEnter={() => setHoveredCard(output.id)}
            onMouseLeave={() => setHoveredCard(null)}
          >
            <div className="text-sm font-medium text-gray-800 mb-3 h-10 flex items-center">
              {output.name}
            </div>
